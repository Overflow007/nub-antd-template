...
function createMockHandler(method, path, value) {
  return function mockHandler(...args) {
    ...
    if (typeof value === 'function') {
      value('non-proxy','non-proxy',...args);//0标识非代理请求
    } else {
      res.json(value);
    }
  };
...
function realApplyMock(devServer) {
  ...
  let lastIndex = app._router.stack.length;
  ...
  app._router.stack.forEach((item, index) => {
    console.log('====router',item.name);
    if (item.name === 'webpackDevMiddleware') {
      lastIndex = index;
    }
  }); */
  const mockAPILength = app._router.stack.length - lastIndex;
  ...
  watcher.on('change', path => {
    ...
    // 删除旧的 mock api
    app._router.stack.splice(lastIndex , mockAPILength);
