...
const BOTH_AREA = 'BOTH_AREA';
const NONE_AREA = 'NONE_AREA';
const DEFAULT_DIRECTION = 'ENTER';
...
class MapSelector extends Component {
  static propTypes = {
    ...
    direction: PropTypes.oneOf(['ENTER', 'LEAVE']),
    adcode: PropTypes.number,
  ...
  static defaultProps = {
    visible: true,
    onSelect: (type, direction, territory, adcode, longitude, latitude, radius) => {
      console.log(type, direction, territory, adcode, longitude, latitude, radius);
    },
    type: BOTH_AREA,
  ...
  showInitMapData = props => {
    const {
      direction, adcode, longitude, latitude, radius,
    } = props;
  ...
  renderContent() {
    return (
      <div id={cx('container')} className={cx('container')}>
