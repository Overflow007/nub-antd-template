...
import './CommandErrorManager.scss';
...
@observer
class CommandErrorManager extends Component {
    ...
    dealExceptions = () => {
        if (this.state.selectedRows[0]) {
            const param = {
                commandQueueIds: this.state.selectedRows[0].id
            };
            ...
            http.post('/call/call.do', {
                ...
                const returnData1 = JSON.parse(returnData.result);
                // console.log(returnData1[this.state.selectedRows[0].id]);
                if (returnData1[this.state.selectedRows[0].id] == '消息重投失败') {
                    message.error('消息重投失败');
                } else {
                    message.success('消息重投成功');
    ...
    loadData = (current, pageSize, pagination) => {
        ...
        return http.post('/call/call.do', {
            bean: 'CommandServ',
            method: 'qryUosCommandErrorsByCond',
            param: param
        }, {
            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
        }).then(res => {
			...
			<div style={{ 'padding': '15px' }}>
				<div className={PREFIX}>
					<StandardList ref={'instanceList'} allowChangeSelectType={true} columns={columns}
								  ...
								  title={'流程消息管理'} rowKey={'processInstanceId'}>
						<div style={{position:'sticky',top:'10px',backgroundColor:'#fff',zIndex: 199,opacity: '1',marginBottom:'10px', display: ((this.state.selectedRows != null && this.state.selectedRows.length > 0) ? 'inline-block' : 'none') }}>
							...
							<Button onClick={this.dealExceptions} type="primary" style={{float: 'right',display: ((this.state.selectedRows != null&&this.state.selectedRows.length > 0)? 'inline' : 'none')}}>消息重投</Button>
						</div>
...
CommandErrorManager.propTypes = {};

CommandErrorManager.defaultProps = {};
